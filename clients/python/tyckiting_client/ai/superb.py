"""
The one after goodradar. Almost final bot before qualifiers.
"""
    ...
    def move(self, bots, events):
        ...
        for i, bot in enumerate(bots, start=1):
            ...
            if bot.alive:
                ...
                # Always flank first
                if self.round_no > 1:
                    last_bot = self.get_last_round().get_bot(bot.bot_id)
                    (ebot, lets_panic) = self.should_i_move(last_bot, self.panic_counter_limit)
                    bot.panic_counter = ebot.panic_counter

                if lets_panic:
                    print "{} PANIC panic detected {}, counter={}".format(bot.bot_id, bot.detected, bot.panic_counter)
